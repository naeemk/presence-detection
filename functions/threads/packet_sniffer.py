...
def packet_sniffer(interface, probelist, sniffercords, lock, sniffercords_ready, measured_power, n, socket, network_ips, my_ip):
        ...
        sniff(iface=interface, prn=lambda packet: process_packet(packet, probelist, sniffercords, measured_power, n, socket, network_ips, my_ip, lock), store=False, lfilter=lambda x: x.type == 0 and x.subtype == 4)
